...
def start_interview(request: StartInterviewRequest):
    ...
    return {"status": "success", "message": f"Interview {request.interview_id} initialized."}


@router.post("/final-report", response_model=FinalReportResponse)
def final_report(request: FinalReportRequest):
    session = sessions.get(request.interview_id, {"scores": [7.5, 8.0, 7.8]})
    avg_score = scoring_service.calculate_overall_score(session["scores"])
    recs = recommendation_service.generate_recommendations(avg_score)
